...
namespace WiMSAPI.Areas.Operation.Controllers
{
    ...
    public class HandlingDocController : Controller
    {
        [HttpGet("{whId}/{storerKey}/{docNo}/{docTypeId}")]
        public IActionResult Get(short whId, string storerKey, string docNo, short docTypeId)
        {
            try
            {
                ...
                return Ok(HandlingDoc.Get(whId, storerKey, docNo, docTypeId, Session.GetUserId(HttpContext)));
            }
            catch (Exception ex) { return Conflict(new ErrorModel { Message = ex.Message, FieldName = "Get" }); }
        }

        [HttpPost]
        public IActionResult Post([FromBody] HandlingDoc asn)
        {
            try
            {
                string controllerName = RouteData.Values["controller"].ToString(); if (Authentication.UserUpdatedSession(Session.GetUserId(HttpContext), controllerName))
                ...
                return Ok(HandlingDoc.Save(asn, Session.GetUserId(HttpContext)));
            }
