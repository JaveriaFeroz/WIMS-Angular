...
let NoteComponent = class NoteComponent {
    //#endregion
    constructor(router, formbulider, svcNote, svcToaster, svcWaitDlg, svcSearchDlg, route, svcRecipient, svcAuth, svcFormSubmission, svcHistoryDlg) {
        this.router = router;
        ...
        this.colCI = [
            {
                ...
                children: [
                    {
                        headerName: "Charge", field: "chargeTypeId", cellEditor: agGridHelper_1.agGridHelper.getAgilitySelect(),
                        cellEditorParams: { source: 'NoteCharge', class: "200" }, valueFormatter: agGridHelper_1.agGridHelper.getNoteChargeName, width: 200
                    },
                    {
        ...
        this.colRI = [
            {
                ...
                children: [
                    {
                        headerName: "Accessorial Charge", field: "chargeTypeId", cellEditor: agGridHelper_1.agGridHelper.getAgilitySelect(),
                        cellEditorParams: { source: 'NoteCharge', class: "200" }, valueFormatter: agGridHelper_1.agGridHelper.getNoteChargeName, width: 200
                    },
                    {
                        headerName: "Qty", field: "quantity", type: "numericColumn", width: 80,
    ...
    tbSave() {
        try {
            ...
            if (!this.frmNote.invalid) {
                var formData = this.frmNote.getRawValue();
                ...
                if (this.errors.length > 0) {
                    ...
                    this.svcWaitDlg.open({});
                    this.svcNote.save(formData).subscribe(data => {
    ...
    tbHistory(formId) {
        try {
            this.svcWaitDlg.open({});
            this.svcRecipient.getHistory(agEnum_1.agEnum.WorkFlow.DebitNote, formId).subscribe(r => {
                this.svcHistoryDlg.open("Debit Note # " + formId, agGridHelper_1.agGridHelper.colHistory, r);
            }, error => { this.svcToaster.showFailure(error); }, () => { this.svcWaitDlg.close(); });
        }
        catch (e) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (stateId == 3 || stateId == 4 || stateId == 99) {
                    recipients = this.svcRecipient.getCreator(agEnum_1.agEnum.WorkFlow.DebitNote, formId);
                }
                rxjs_1.forkJoin([recipients]).subscribe(results => {
                    ...
                    if (recipients === undefined || recipients.length == 0) {
                        ...
                        this.svcFormSubmission.open(agEnum_1.agEnum.getInvoiceState(nextStateId) + " - Debit Note # " + this.frmNote.controls.invoiceNo.value, agEnum_1.agEnum.getInvoiceState(nextStateId), recipients);
        ...
        try {
            if (this.goCI.api.getSelectedRows().length > 0) {
                if (confirm("Are you sure you want to Delete selected row?")) {
                    ...
                    agGridHelper_1.agGridHelper.setGridDeleteFilter(this.goCI.api);
                }
                this.setFooter();
            }
            else
        ...
        try {
            if (this.goRI.api.getSelectedRows().length > 0) {
                if (confirm("Are you sure you want to Delete selected row?")) {
                    ...
                    agGridHelper_1.agGridHelper.setGridDeleteFilter(this.goRI.api);
                }
                this.setFooter();
            }
            else
    ...
    validateGridStatus() {
        var rd = (document.querySelector('[id="btnEdit"]')['disabled'] == false || document.querySelector('[id="btnAdd"]')['disabled'] == false);
    ...
    get(Id) {
        try {
            this.svcWaitDlg.open({});
            this.svcNote.get(Id, this.workFlowId).subscribe(note => {
                if (note) {
                    this.frmNote.disable();
                    ...
                    note.stateName = agEnum_1.agEnum.getInvoiceState(note.stateId);
    ...
    loadInvoiceDetail(refInvoiceNo) {
        try {
            if (refInvoiceNo) {
                this.svcWaitDlg.open({});
                this.svcNote.getRefInvInfo(refInvoiceNo).subscribe(inv => {
                    if (inv) {
                        ...
                        this.svcToaster.showWarning("The provided Invoice # doesn't exist or you don't have permission to refer that invoice");
                    }
                }, error => { this.svcToaster.showFailure(error); }, () => { this.svcWaitDlg.close(); });
            }
        }
    ...
    loadLookup() {
        try {
            this.svcWaitDlg.open({});
            this.svcNote.getLookup().subscribe(data => {
...
__decorate([
    core_1.ViewChild('refInvoiceNo', { static: true })
], NoteComponent.prototype, "refInvoiceNo", void 0);
__decorate([
    core_1.ViewChild('invoiceNo', { static: true })
], NoteComponent.prototype, "invoiceNo", void 0);
__decorate([
    core_1.ViewChild('remarks', { static: true })
], NoteComponent.prototype, "remarks", void 0);
