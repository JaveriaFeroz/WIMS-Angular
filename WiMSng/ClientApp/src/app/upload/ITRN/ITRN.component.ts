...
import { ITRNService } from './ITRN.service';
...
export class ITRNComponent implements OnInit {
  //#region form variables
  readonly optionName: string = 'Inventory Transaction';
  frmITRN: any;
  lstWarehouse: any;
  wipDocument: File;
  errors: string[] = [];
  ...
  constructor(private router: Router, private formbulider: FormBuilder,
    private svcITRN: ITRNService, private svcToaster: agToasterService, private svcWaitDlg: WaitDialogService) {
  }
  ...
  tbSave() {
    try {
      ...
      if (!this.frmITRN.invalid) {
        var formData: ITRN = this.frmITRN.getRawValue();
        ...
        fileReader.onload = (e) => {
          ...
          if (this.errors.length > 0) {
            ...
            var ITRNs: ITRN[] = this.getData(jsonData, jsonData[0].length);
            if (this.errors.length == 0)
              this.validate(ITRNs);
            if (this.errors.length > 0) {
              this.errors;
  ...
  getData(jsonData: any[][], headerLength: number) {
    this.errors = [];
    var ITRNs: ITRN[] = [], proNumber = "", itrn: ITRN = new ITRN();
  ...
  private loadLookup() {
    try {
      this.svcITRN.getLookUp().subscribe(
