...
export class LocationComponent implements OnInit {
  ...
  constructor(private router: Router, private formbulider: FormBuilder,
    private svcLC: LocationService, private svcToaster: agToasterService, private svcWaitDlg: WaitDialogService) {
  }

  ngOnInit() {
    this.frmLocation = this.formbulider.group({
      //whId: [null, [Validators.required]],
      //storerKey: [null, [Validators.required]],
      //TransactionDate: [null, [Validators.required]]
    });
    //this.loadLookup();
    this.frmLocation.disable();
  ...
  tbAdd() {
    this.frmLocation.reset();
    this.frmLocation.enable();
  ...
  tbSave() {
    try {
      this.frmLocation.markAllAsTouched();
      if (!this.frmLocation.invalid) {
        var formData: Location = this.frmLocation.getRawValue();
        ...
        fileReader.onload = (e) => {
          ...
          if (this.errors.length > 0) {
            ...
            var Locations: Location[] = this.getData(jsonData, jsonData[0].length);
            if (this.errors.length == 0)
              this.validate(Locations);
            if (this.errors.length > 0) {
              this.errors;
              return;
            }
              ...
              this.svcLC.save(Locations).subscribe(
                () => {
                  this.initForm();
                  agFormHelper.setFormControls(this.optionName, agFormMode.Initialize);
  ...
  getData(jsonData: any[][], headerLength: number) {
    this.errors = [];
    var lcs: Location[] = [], proNumber = "", LC: Location = new Location();
    for (let i = 1; i < jsonData.length; i++) {
      ...
      if (currentRecord.length == headerLength) {
        ...
        //}
        LC = new Location();
        LC.Location = currentRecord[0].trim();
        LC.LocCategory = currentRecord[1].trim();
        LC.WMSLcCode = currentRecord[2].trim();
        LC.Is10x = currentRecord[3].trim();
