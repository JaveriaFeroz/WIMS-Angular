...
import { StorerService } from './StorerService.service';
...
export class StorerComponent implements OnInit {
  ...
  constructor(private router: Router, private formbulider: FormBuilder,
    private svcLC: StorerService, private svcToaster: agToasterService, private svcWaitDlg: WaitDialogService) {
  }
  ...
  tbSave() {
    try {
      ...
      if (!this.frmStorer.invalid) {
        var formData: Storer = this.frmStorer.getRawValue();
        ...
        fileReader.onload = (e) => {
          ...
          if (this.errors.length > 0) {
            ...
            var Storers: Storer[] = this.getData(jsonData, jsonData[0].length);
            if (this.errors.length == 0)
  ...
  getData(jsonData: any[][], headerLength: number) {
    this.errors = [];
    var lcs: Storer[] = [], proNumber = "", LC: Storer = new Storer();
