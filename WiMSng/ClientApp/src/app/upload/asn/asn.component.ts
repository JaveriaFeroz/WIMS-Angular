...
import { asn } from './asn';
import { asnService } from './asnService.service';
...
export class asnComponent implements OnInit {
  ...
  readonly optionName: string = 'Advance Shipment Notice';
  frmasn: any;
  lstWarehouse: any;
  wipDocument: File;
  errors: string[] = [];
  ...
  constructor(private router: Router, private formbulider: FormBuilder,
    private svcLC: asnService, private svcToaster: agToasterService, private svcWaitDlg: WaitDialogService) {
  }
  ...
  tbSave() {
    try {
      ...
      if (!this.frmasn.invalid) {
        var formData: asn = this.frmasn.getRawValue();
        ...
        fileReader.onload = (e) => {
          ...
          if (this.errors.length > 0) {
            ...
            var asns: asn[] = this.getData(jsonData, jsonData[0].length);
            if (this.errors.length == 0)
              this.validate(asns);
            if (this.errors.length > 0) {
              this.errors;
              ...
              this.svcLC.save(asns).subscribe(
                () => {
                  this.initForm();
                  agFormHelper.setFormControls(this.optionName, agFormMode.Initialize);
  ...
  getData(jsonData: any[][], headerLength: number) {
    this.errors = [];
    var lcs: asn[] = [], proNumber = "", LC: asn = new asn();
    for (let i = 1; i < jsonData.length; i++) {
      ...
      if (currentRecord.length == headerLength) {
        ...
        //}
        LC = new asn();
        LC.ReceiptKey = currentRecord[0].trim();
        LC.Storerkey = currentRecord[1].trim();
        LC.ExternReceiptKey = currentRecord[2].trim();
        LC.ContainerType = currentRecord[3].trim();
        LC.ContainerQty = currentRecord[4].trim();
        LC.WhseID = currentRecord[5].trim();
        LC.ReceiptDate = currentRecord[6].trim();
  ...
  private validate(lc: asn[]) {
    this.errors = [];
    //if (st.some(x => x.proNo == "")) {
    //  this.errors.push('No row in Storage Upload can have empty PRO Number');
    //}
    //if (st.some(y => y.details.some(x => x.asn == ""))) {
    //  this.errors.push('asn is a required field and must be provided in each row of upload sheet');
    //}
