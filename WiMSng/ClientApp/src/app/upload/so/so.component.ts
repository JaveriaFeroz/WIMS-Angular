...
import { so } from './so';
import { soService } from './soService.service';
...
export class soComponent implements OnInit {
  ...
  constructor(private router: Router, private formbulider: FormBuilder,
    private svcLC: soService, private svcToaster: agToasterService, private svcWaitDlg: WaitDialogService) {
  }
      ...
      if (!this.frmso.invalid) {
        ...
        fileReader.onload = (e) => {
          ...
          if (this.errors.length > 0) {
            ...
            var sos: so[] = this.getData(jsonData, jsonData[0].length);
            if (this.errors.length == 0)
              this.validate(sos);
            if (this.errors.length > 0) {
              this.errors;
  ...
  getData(jsonData: any[][], headerLength: number) {
    this.errors = [];
    var lcs: so[] = [], proNumber = "", LC: so = new so();
