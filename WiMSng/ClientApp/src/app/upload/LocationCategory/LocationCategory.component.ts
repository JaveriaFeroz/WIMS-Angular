...
import { LocationCategoryService } from './LocationCategoryservice.service';
import { LocationCategoryDetail } from './LocationCategorydetail';

@Component({
  selector: 'app-st',
  templateUrl: './LocationCategory.component.html',
  styleUrls: ['./LocationCategory.component.css']
})
export class LocationCategoryComponent implements OnInit {
  //#region form variables
  readonly optionName: string = 'Location Category';
  frmLocationCategory: any;
  lstWarehouse: any;
  wipDocument: File;
  errors: string[] = [];
  ...
  constructor(private router: Router, private formbulider: FormBuilder,
    private svcLC: LocationCategoryService, private svcToaster: agToasterService, private svcWaitDlg: WaitDialogService) {
  }

  ngOnInit() {
    this.frmLocationCategory = this.formbulider.group({
      //whId: [null, [Validators.required]],
      //storerKey: [null, [Validators.required]],
      //TransactionDate: [null, [Validators.required]]
    });
    //this.loadLookup();
    this.frmLocationCategory.disable();
  ...
  tbAdd() {
    this.frmLocationCategory.reset();
    this.frmLocationCategory.enable();
  ...
  tbSave() {
    try {
      this.frmLocationCategory.markAllAsTouched();
      if (!this.frmLocationCategory.invalid) {
        var formData: LocationCategory = this.frmLocationCategory.getRawValue();
        ...
        fileReader.onload = (e) => {
          ...
          if (this.errors.length > 0) {
            ...
            var LocationCategorys: LocationCategory[] = this.getData(jsonData, jsonData[0].length);
            if (this.errors.length == 0)
              this.validate(LocationCategorys);
            if (this.errors.length > 0) {
              this.errors;
              return;
            }
              ...
              this.svcLC.save(LocationCategorys).subscribe(
                () => {
                  this.initForm();
                  agFormHelper.setFormControls(this.optionName, agFormMode.Initialize);
  ...
  getData(jsonData: any[][], headerLength: number) {
    this.errors = [];
    var lcs: LocationCategory[] = [], proNumber = "", LC: LocationCategory = new LocationCategory();
    for (let i = 1; i < jsonData.length; i++) {
      ...
      if (currentRecord.length == headerLength) {
        ...
        LC.WMSLcCode = currentRecord[0].trim();
        LC.CategoryName = currentRecord[1].trim();
        LC.Is10x = currentRecord[2].trim();
  ...
  private validate(lc: LocationCategory[]) {
    this.errors = [];
