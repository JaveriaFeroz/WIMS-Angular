...
import { SKU } from './SKU';
import { SKUService } from './SKUService.service';
...
export class SKUComponent implements OnInit {
  ...
  readonly optionName: string = 'SKU';
  frmSKU: any;
  lstWarehouse: any;
  wipDocument: File;
  errors: string[] = [];
  ...
  constructor(private router: Router, private formbulider: FormBuilder,
    private svcLC: SKUService, private svcToaster: agToasterService, private svcWaitDlg: WaitDialogService) {
  }
      ...
      if (!this.frmSKU.invalid) {
        ...
        fileReader.onload = (e) => {
          ...
          if (this.errors.length > 0) {
            ...
            var SKUs: SKU[] = this.getData(jsonData, jsonData[0].length);
            if (this.errors.length == 0)
              this.validate(SKUs);
            if (this.errors.length > 0) {
              this.errors;
              ...
              this.svcLC.save(SKUs).subscribe(
                () => {
                  this.initForm();
                  agFormHelper.setFormControls(this.optionName, agFormMode.Initialize);
  ...
  getData(jsonData: any[][], headerLength: number) {
    this.errors = [];
    var lcs: SKU[] = [], proNumber = "", LC: SKU = new SKU();
    for (let i = 1; i < jsonData.length; i++) {
      ...
      if (currentRecord.length == headerLength) {
        ...
        LC.SKU = currentRecord[1].trim();
        LC.Description = currentRecord[2].trim();
        LC.Packkey = currentRecord[3].trim();
        LC.GrossWeight = currentRecord[4].trim();
        LC.NetWeight = currentRecord[5].trim();
        LC.Cube = currentRecord[6].trim();
        LC.Litre = currentRecord[7].trim();
        LC.SkuGroup = currentRecord[8].trim();
        LC.Is10x = currentRecord[9].trim();
